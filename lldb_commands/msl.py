...
def processStackTraceStringFromAddresses(frameAddresses, target):
    ...
    for index, frameAddr in enumerate(frameAddresses):
        ...
        i = ds.attrStr(
            "frame #{:<2}: {} ".format(index, hex(addr.GetLoadAddress(target))), "grey"
        )
        frame_string += "{} {}`{} {}\n".format(
            i,
            ds.attrStr(str(addr.module.file.basename), "cyan"),
            ds.attrStr(str(name), "yellow")
            if not symbol.IsSynthetic()
            else ds.attrStr(str(name), "red"),
            offset_str,
        )
...
def generateScript(addr, options):
    script = (
        "@import ObjectiveC;  mach_vm_address_t addr = (mach_vm_address_t)"
        + str(addr)
        + ";\n"
    )
    script += r"""
