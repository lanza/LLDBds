...
def handle_command(debugger, command, exe_ctx, result, internal_dict):
    ...
    if options.search_functions:
        ...
        if symList.GetSize() == 0:
            result.SetError(
                ds.attrStr(
                    'Couldn\'t find any matches for "{}"'.format(cleanCommand), "red"
                )
            )
            return
        sym = symList
...
def generateAssemblyFromSymbol(sym, options, exe_ctx):
    ...
    output = (
        ds.attrStr(str(sym.addr.module.file.basename) + ", ", "cyan")
        + ds.attrStr(str(sym.name), "yellow")
        + "\n"
    )
    counter = 0
    ...
    if len(instructions) == 0:
        return
    startAddress = (
        instructions.GetInstructionAtIndex(0).GetAddress().GetLoadAddress(target)
    )
    ...
    for inst in instructions:
        ...
        loadaddr = ds.attrStr(
            hex(inst.addr.GetLoadAddress(target)) + (" <+" + offset + ">:").ljust(8),
            "grey",
        )
        ...
        if (
            "call" in mnemonicStr
            and inst.GetOperands(target).startswith("0x")
            and len(inst.GetOperands(target).split(",")) == 1
        ):
...
def generateDescriptionByAddress(addr, target):
    ...
    if "__DATA.__objc_selrefs" in name:
        ...
        newAddr = target.ResolveLoadAddress(
            addr.GetLoadAddress(target) + 0x10
        )  # TODO, do 32 bit
