...
def makeSureEverythingIsOK(result):
    ...
    if ds_has_run_once is False:
        ...
        if subprocess.call(["/usr/bin/which", "jtool"], shell=False) != 0:
            result.SetError(
                "Can't find jtool in PATH or jtool isn't installed (http://www.newosxbook.com/tools/jtool.html), you can determine this in LLDB via \"(lldb) script import os; os.environ['PATH']\"\nYou can persist this via (lldb) script os.environ['PATH'] += os.pathsep + /path/to/jtool/folder"
            )
            return 1
        ...
        ds_jtool_path = (
            subprocess.Popen(
                ["/usr/bin/which", "jtool"], shell=False, stdout=subprocess.PIPE
            )
            .communicate()[0]
            .rstrip("\n\r")
        )
...
def get_cputype_string(target, addr):
    ...
    cpu_val = target.CreateValueFromAddress(
        "__unused", target.ResolveLoadAddress(cputype_addr), int32_t_ptr_type
    )
...
def handle_command(debugger, command, exe_ctx, result, internal_dict):
    ...
    if len(args) == 0:
        ...
        result.AppendMessage(
            "Inspecting main executable, {}{}{}".format(
                "" if isXcode() else "\033[36m",
                module.GetFileSpec().basename,
                "" if isXcode() else "\033[0m",
            )
        )
        ...
        if module is None:
            ...
            try:
                ...
                try:
                    ...
                    result.SetError(
                        'Unable to find module "{}", use "image list -f ModuleName"'.format(
                            args[0]
                        )
                    )
                    return
            ...
            if addr.module.IsValid() == False:
                result.SetError(
                    "Unable to find module for address {}".format(hex(address))
                )
                return
