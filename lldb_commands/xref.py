...
def handle_command(debugger, command, exe_ctx, result, internal_dict):
    ...
    loadOffset = (
        ds.getSection(module=executablePath, name="__TEXT").addr.GetLoadAddress(target)
        - pagesize
    )
    ...
    for i, m in enumerate(matches):
        ...
        try:
            ...
            outputStr += "error: at {} {}".format(
                res.group("addr"), res.group("offset")
            )
            continue
        ...
        if searchAddr == potential:
            ...
            if res.group("initial"):
                ...
                resolved = (
                    int(regex.match(matches[i - 1]).group("addr"), 16) + loadOffset
                )
...
def getObjcMethNameAddress(addr, target):
    ...
    for i, x in enumerate(dataArray):
        ...
        summary = target.CreateValueFromAddress(
            "somename", straddr, charPointerType
        ).summary
