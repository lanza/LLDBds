...
def handle_command(debugger, command, exe_ctx, result, internal_dict):
    ...
    if GlobalProcess.hasPerformedSetup is False:
        setupIfiOS11(target)
        debugger.HandleCommand(
            "exp -lobjc -O -- [UIDebuggingInformationOverlay prepareDebuggingOverlay]"
        )
        ...
        debugger.HandleCommand(
            'exp -lobjc -O -- [[UIDebuggingInformationOverlay overlay] toggleVisibility]; [UIDebuggingInformationOverlay overlay] ?  @"Displaying UIDebuggingInformationOverlay... resume execution" : @"Failure, womp"'
        )
...
def setupIfiOS11(target):
    ...
    versionval = target.EvaluateExpression(
        "[[UIDevice currentDevice] systemVersion]", options
    )
...
def tweakiOS11Memory(target, moduleName):
    ...
    symbolsFF = [
        "__fetchedInternalDeviceOnceToken",
    ...
    for symStr in symbolsFF:
        ...
        for symbol in syms.symbols:
            ...
            size = (
                symbol.GetEndAddress().GetFileAddress()
    ...
    syms = module.FindSymbols(
        "__hasCachedDebuggingOverlayEnabled", lldb.eSymbolTypeData
    )
    for symbol in syms.symbols:
        loadAddr = symbol.GetStartAddress().GetLoadAddress(target)
        size = (
