...
def handle_command(debugger, command, exe_ctx, result, internal_dict):
    ...
    copycommand = 'echo "{} {}  {}" | pbcopy'.format(
        source.replace("$", "\$"), cmd, stderr
    )
    os.system(copycommand)
    if options.debug_with_clipboard:
        result.AppendMessage(
            "Copied dryrun script to clipboard... paste in Terminal. Will ensure it compiles then exit"
        )
    elif options.listprobes:
        result.AppendMessage(
            "Copied listing script to clipboard... paste in Terminal. Will only list probes then exit"
        )
    else:
...
def generateDTraceScript(options):
    headers = "#!/usr/sbin/dtrace -{}s".format(
        "e" if options.debug_with_clipboard else "l" if options.listprobes else ""
    )
    script = headers + "\n\n"
...
"""
    ...
    script += "{}:{}:{}:{} {}".format(
        options.provider, options.module, options.function, options.name, predicate
    )
    script += """
{{
    {}
