...
def __lldb_init_module(debugger, internal_dict):
    debugger.HandleCommand("command script add -f breakifonfunc.breakifonfunc biof")
...
def breakifonfunc(debugger, command, exe_ctx, result, internal_dict):
    ...
    if not breakpoint.IsValid() or breakpoint.num_locations == 0:
        result.AppendWarning(
            "Breakpoint isn't valid or hasn't found any hits: " + clean_command[0]
        )
    else:
        result.AppendMessage(
            '"{}" produced {} hits\nOnly will stop if the following stack frame symbols contain:\n{}` "{}" produced {} hits'.format(
                clean_command[0],
                breakpoint.num_locations,
...
def breakpointHandler(frame, bp_loc, dict):
    if len(GlobalOptions.symbols) == 0:
        print(
            "womp something internal called reload LLDB init which removed the global symbols"
        )
        return True
